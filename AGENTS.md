...
This is a Joget DX9 marketplace plugin (OSGi bundle) that adds a single Datalist Column Formatter: **Options Template Datalist Formatter**. It takes a semicolon-separated field value, resolves each individual value to a label and grouping (via a static options grid or a configured Form Options Binder), and renders each resolved value through a user-configurable HTML template (`{id}` / `{label}` / `{grouping}` placeholders), joined by a configurable separator.
...
- [OptionsTemplateDatalistFormatter.java](src/main/java/org/joget/marketplace/OptionsTemplateDatalistFormatter.java) — extends `DataListColumnFormatDefault`, the actual formatter logic:
  - `format()` splits the incoming cell value on `;`, trims each token, looks up a label via `getOptionMap()` and a grouping via `getGroupingMap()`, and substitutes `{id}`/`{label}`/`{grouping}` into the configured `template` string, joining results with `separator`.
  - `getOptionMap()` resolves value→label pairs from two possible sources, in this order of precedence at runtime: first tries the static `options` grid property, then — if an `optionsBinder` is configured — loads a `FormLoadOptionsBinder` plugin instance via `PluginManager` and overwrites `optionMap` with rows loaded from `FormLoadBinder.load()`. The same binder pass also populates the parallel `groupingMap` (value→grouping), read via `getGroupingMap()` — grouping is only available when the binder returns a `FormUtil.PROPERTY_GROUPING` property (or a third positional property) per row; the static `options` grid has no grouping column, so `{grouping}` resolves to an empty string for those entries. Both maps are cached per-instance (lazy-loaded once, not per-row) and rebuilt together whenever the binder path runs.
